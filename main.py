...
# Load Model Weights (Change device="0" if using an NVIDIA GPU)
DEVICE = "cpu"
ir_model = YOLO("models/thermal_best.pt")
rgb_model = YOLO("models/rgb_best.pt")

# Define Pre-recorded Sensor Data Sources
ir_source = "IR_videos/ir_test.mp4"
rgb_source = "RGB_videos/rgb_test1.mp4"
...
# Tracking State Class with Clutter Filtering
class TrackerState:
  ...
  def __init__(self):
    ...
    # To track stationary clutter: map center -> frame count of zero movement
...
def process_pipeline_logic(boxes_data, state, stream_label="Target"):
  """Optimized handler with stationary background clutter rejection."""
  boxes = []
  confs = []
  ...
  if len(boxes) > 0:
    # --- STATIONARY CLUTTER FILTER (Mainly for IR) ---
    if stream_label == "IR" and len(boxes) > 0:
      filtered_boxes = []
      filtered_confs = []
      for b, c in zip(boxes, confs):
        curr_center = (int((b[0] + b[2]) / 2), int((b[1] + b[3]) / 2))
        
        # Check if this box is in the exact same spot as the previous frame's center
        if state.last_center is not None:
          dist = np.hypot(curr_center[0] - state.last_center[0], curr_center[1] - state.last_center[1])
          # If it moved less than 2 pixels, it's likely a static hot rock/roof
          if dist < 2.0:
            state.static_counter += 1
          else:
            state.static_counter = 0
        
        # If a target stays completely static for more than 10 frames, ignore it as clutter
        if state.static_counter < 10:
    ...
    if len(boxes) == 0:
      # If all boxes were filtered out as static clutter, treat as searching/coasting
      if state.last_box is not None and state.missing_frames < state.max_coast_frames:
    ...
    if len(boxes) == 1:
      # Scenario A: Exactly 1 Target -> Direct Single Target Lock
      ...
      # Scenario B: Multiple Drones -> Check for Swarm Cluster vs Scattered Drones
      ...
      if len(unique_labels) > 0:
        # Sub-scenario B1: A dense swarm cluster exists
        ...
        # Sub-scenario B2: Multiple scattered drones -> Sticky Nearest-Neighbor Matching
        if state.last_center is not None:
          distances = [
              np.hypot(
        ...
        state.status = "SINGLE LOCKED"
        return state.last_box, state.last_center, state.status, "SINGLE", [best_box]
  else:
    # Scenario C: No Targets detected -> Apply Coasting Grace Period
    if state.last_box is not None and state.missing_frames < state.max_coast_frames:
      state.missing_frames += 1
      state.status = f"COASTING ({state.missing_frames}/{state.max_coast_frames})"
      return state.last_box, state.last_center, state.status, "COAST", []
...
while cap_ir.isOpened() and cap_rgb.isOpened():
  ...
  # --- Advance IR Stream Independently with Tracking ---
  if current_time >= next_ir_time:
    ...
    if frame_ir is not None:
      ...
      res_ir = ir_model.track(
          frame_ir,
          conf=0.4,  # Slightly raised confidence to cut down weak background noise
          persist=True,
  ...
  # --- Advance RGB Stream Independently with Tracking ---
  ...
  # Ensure frames exist before processing
  if frame_ir is None or frame_rgb is None:
    break

  # Create working copies for drawing
  display_ir = frame_ir.copy()
  display_rgb = frame_rgb.copy()

  # --- Process IR Pipeline Logic ---
  ...
  # --- Professional On-Screen Overlays (HUD Style) ---
  cv2.putText(
      display_ir, "SENSOR 01: THERMAL IR", (15, 30),
      cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2
  )
  cv2.putText(
      display_rgb, "SENSOR 02: OPTICAL RGB", (15, 30),
      cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2
  )

  cv2.putText(
      display_ir, f"STATUS: {ir_status}", (15, 60),
      cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 0), 2
  )
  cv2.putText(
      display_rgb, f"STATUS: {rgb_status}", (15, 60),
      cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 0), 2
  )

  # --- Construct Professional GCS Dashboard Layout ---
  ...
  cv2.putText(
      banner, "AEGIS C-UAS COMMAND & CONTROL SYSTEM", (20, 28),
      cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2
  )
  cv2.putText(
      banner, f"SYS TIME: {timestamp_str} | MODE: ACTIVE TRACKING", (FRAME_W + 40, 28),
